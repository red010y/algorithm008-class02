...
回溯的解决方法，也就是穷举搜索所有可能的装法，然后找出满足条件的最大值。不过，回溯算法的复杂度比较高，是指数级别的。那有没有什么规律，可以有效降低时间复杂度呢？我们一起来看看。
...
记录表什么都不返回，我们最终的结果不是最大重量么？最大重量在那？并且返回了之后的数还算么？最大重量记录在变量max里。之后的数还算。还是那个思路，从当前的i往前算（7，0）一直往前加上8的cw，也就是放入后一个，在放下一个。这个时候如果已经放过了（2，2）就不用递归了，进行下一个（1，2）的计算。
...
00那特殊了？00肯定是true。
那00后面干嘛要在进行判断？那是加上了第一个物品的重量
都说了不加入，那干嘛还有一个循环判断。开始第二行后，把前一行的true复制过来用。确实没有加入新的值。
后面加入的代码，加入就加入怎么还得循环判断？得确保之前为true的都加这个新值啊。
不考虑新的物品放在一个他自身重量的位置么？肯定不考虑，你第一次放入你一个物品，之后的物品都在前面的基础上进行累加。
...
干嘛要j＝w-item[i]？进行剪枝，一维数组上item[i]这个值占具了位置，没必要在从这开始。
如果剪枝后的值存在，就和当前的值相加，新的值就标记true。
最后输出最大值。
...
怎么保证价值最大？每次向前加，都会记录一次最大值。保存在变量中。
...
j <= w-weight[i]；进行一个剪枝，已经选择了weight[i]，并且不能超过总重量，剩下的取值范围肯定在w-weight[i]范围内。
v > states[i][j+weight[i]]；就是在这里进行的覆盖小值的操作，如果如来这个位置上的值小于了现在的值就进行覆盖。
找出最大值，为什么从0开始？不应该从最大的开始么？这里应该可以优化
时间复杂度是 O(n*w)，空间复杂度也是 O(n*w)。跟上一个例子类似，空间复杂度也是可以优化的，你可以自己写一下。
...
j-items[i]；j是最小的花费，减去当前商品后哪个值在二维数组中存在，就说明这个当前的商品可购买。
i-1？为什么非要-1，不-1也可行啊
为什么！=0要打印items[0]?
